    ...
    var request = https.request(options, function(resp) {
      if (resp.statusCode == 200) {
        ...
        gunzip.on('data', function(data) {
          ...
          fs.writeFileSync(path.resolve(__dirname,'../../tmp/config.json'), buffer.join(''));
          console.log('Retrieved template has been written to ../../tmp/config.json');
...
function publishTemplate(etag) {
  getAccessToken().then(function(accessToken) {
    ...
    request.write(fs.readFileSync(path.resolve(__dirname,'../../tmp/config.json'), 'UTF8'));
    request.end();
  });
}
